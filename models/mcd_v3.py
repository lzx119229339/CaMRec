...
class MCD_V3(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MCD_V3, self).__init__(config, dataset)
    ...
    def forward(self,t_feat_ori,v_feat_ori,t_generate_v=None,v_generate_t=None):
        ...
        ### 加权融合 
        ### 只在sample阶段用v_generate_t 
        if t_generate_v is not None and v_generate_t is not None: 
            self.t_embedding = self.w1 * v_generate_t + (1-self.w1)*t_unique
            
            self.v_embedding = self.w2 * t_generate_v + (1-self.w2)*v_unique 
      
        else:
            self.t_embedding = t_unique
            self.v_embedding = v_unique 
    ...
    def calculate_loss(self, interaction):
        ...
        diff_loss1,t_generate_v = self.diffusion_tgi.p_losses(v_feat_ori,t_feat_ori,t) ## 用文本生成的图像表征 在图像空间里
        diff_loss2,v_generate_t = self.diffusion_igt.p_losses(t_feat_ori,v_feat_ori,t) ## 用图像生成的文本表征 在文本空间里 
        diff_loss = self.diff_weight*(diff_loss1+diff_loss2)
        
        self.forward(t_feat_ori,v_feat_ori)
    ...
    def sample(self):
        ...
        t_feat_ori = self.text_trs(self.t_feat)
        v_feat_ori = self.image_trs(self.v_feat)
        t_generate_v = self.diffusion_tgi.sample(v_feat_ori,t_feat_ori) 
        v_generate_t = self.diffusion_igt.sample(t_feat_ori,v_feat_ori)
