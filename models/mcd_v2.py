...
class MCD_V1(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MCD_V1, self).__init__(config, dataset)
    ...
    def forward(self,t_feat_ori,v_feat_ori,t_share_v,v_share_t):
        ...
        if t_share_v is not None and v_share_t is not None:
            # print("-"*100)
            # print("共享表征存在")
            self.t_embedding = self.w1 * t_share_v + (1-self.w1)*t_unique
            self.v_embedding = self.w2 * v_share_t + (1-self.w2)*t_unique 
        else:
            # print("-"*100)
            # print("共享表征为None")
    ...
    def calculate_loss(self, interaction):
        ...
        diff_loss1,v_share_t = self.diffusion_tgi.p_losses(v_unique,t_unique,t) 
        diff_loss2,t_share_v = self.diffusion_igt.p_losses(t_unique,v_unique,t)
        
        self.forward(t_share_v,v_share_t)
    ...
    def full_sort_predict(self, interaction):
        ...
        self.forward(t_unique,v_unique,self.sample_tv,self.sample_vt)
    ...
    def sample(self):
        ...
        v_share_t = self.diffusion_tgi.sample(v_unique,t_unique) 
        t_share_v = self.diffusion_igt.sample(t_unique,v_unique)
