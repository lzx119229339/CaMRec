...
class diffusion(nn.Module):
    ...
    def p_losses(self, x1_start, x2_start,x3_start,t, noise=None, loss_type="l2"):
        #
        ...
        diff_i = torch.cat([x_noisy.unsqueeze(1), x2_start.unsqueeze(1),x3_start.unsqueeze(1), t_emb.unsqueeze(1)], dim=1)
    ...
    @torch.no_grad()
    def p_sample(self, x1_t, x2_t,x3_t, t, t_index):
        ...
        diff_i = torch.cat([x1_t.unsqueeze(1), x2_t.unsqueeze(1),x3_t.unsqueeze(1), t_emb.unsqueeze(1)], dim=1)
        x1_start = self.selfAttention(diff_i)
    ...
    @torch.no_grad()
    def sample(self, x1_start, x2_start,x3_start):
        ...
        for n in reversed(range(0, self.timesteps)):
            x1_t = self.p_sample(x1_t,x2_t,x3_t,
