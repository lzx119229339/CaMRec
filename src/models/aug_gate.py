...
class AUG_GATE(GeneralRecommender):
    def __init__(self, config, dataset):
        super(AUG_GATE, self).__init__(config, dataset)
    ...
    def _init_gate_weights(self):
        ...
        nn.init.kaiming_normal_(
            self.gate_layer[0].weight, 
            mode='fan_out', 
            nonlinearity='relu'
        )
        nn.init.constant_(self.gate_layer[0].bias, 0.1)
        
        # 第二层初始化（偏向初始均衡）
        nn.init.constant_(self.gate_layer[2].weight, 0.)
        nn.init.constant_(self.gate_layer[2].bias, 0.)  # 初始gate≈0.5
