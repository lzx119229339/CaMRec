...
class MCD_AUG_V1(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MCD_AUG_V1, self).__init__(config, dataset)
    ...
    # def InfoNCE_Modified(view1, view2, index, temp=0.2):
    #     # 取出 index 对应的样本
    #     view1 = view1[index]
    #     view2 = view2[index]
    ...
    #     # 计算负样本的相似度（仅在 view1 之间计算）
    #     neg_score = torch.matmul(view1, view1.transpose(0, 1))  # 计算 view1 之间的相似性
    #     neg_score = torch.exp(neg_score / temp).sum(dim=1) - torch.exp(torch.diag(neg_score) / temp)  # 移除自身

    #     # 计算对比损失
    #     cl_loss = -torch.log(pos_score / neg_score)
    #     return torch.mean(cl_loss)
    ...
    def forward(self,t_feat_ori,v_feat_ori,t_share_v,v_share_t):
        ...
        """
        1. 先过MLP
        2. id加噪,t作为条件,去帮助恢复噪声,得到predicted_x 
        3. 用mm adj算出多模态item graph的 h_item 
        4. predicted_x和h_item做加权融合? 这一步是为啥 
        5. 部分的h_item换成predicted_x 这样做卷积貌似也可以 
        6. 
        """
        ...
        self.t_embedding = self.w1 * t_share_v + (1-self.w1)*t_unique
        self.v_embedding = self.w2 * v_share_t + (1-self.w2)*v_unique 

        
        self.t_rep,self.t_aug = self.aug(self.t_prefer,self.t_embedding,self.text_graphlearner)
        self.v_rep,self.v_aug = self.aug(self.v_prefer,self.v_embedding,self.image_graphlearner)

        self.result_embed = torch.cat((self.t_rep,self.v_rep),dim=1)
        return None
        # return diff_loss_t2i,diff_loss_i2t
    ...
    def calculate_loss(self, interaction):
        ...
        diff_loss1,v_share_t = self.diffusion_tgi.p_losses(v_feat_ori,t_feat_ori,t) 
        diff_loss2,t_share_v = self.diffusion_igt.p_losses(t_feat_ori,v_feat_ori,t)
    
        
        self.forward(t_feat_ori,v_feat_ori,t_share_v,v_share_t)
        diff_loss = self.diff_weight*(diff_loss1+diff_loss2)
        
        
        loss_rec_main = self.BPR(self.result_embed)
        ...
        ssl_loss_t = self.InfoNCE(self.t_rep,self.t_aug,self.user_nodes) + self.InfoNCE(self.t_rep,self.t_aug,self.pos_item_nodes)
        ssl_loss_v = self.InfoNCE(self.v_rep,self.t_aug,self.user_nodes) + self.InfoNCE(self.v_rep,self.v_aug,self.pos_item_nodes)
        ssl_loss = self.ssl_weight * (ssl_loss_t+ssl_loss_v)
    ...
    def full_sort_predict(self, interaction):
        ...
        # if self.sample_tv is not None and self.sample_vt is not None:
        #     print("-"*100)
        #     print("sample的共享表征存在")
    ...
    def sample(self):
        ...
        t_feat_ori = self.text_trs(self.t_feat)
        v_feat_ori = self.image_trs(self.v_feat)
        v_share_t = self.diffusion_tgi.sample(v_feat_ori,t_feat_ori) 
        t_share_v = self.diffusion_igt.sample(t_feat_ori,v_feat_ori)


        self.sample_tv = t_share_v.detach()
        self.sample_vt = v_share_t.detach()
