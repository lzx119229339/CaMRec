...
"""
这个版本只是比V4多了一个InfoNCE的损失,相当于CLIP,要求文本条件和图像条件对齐 

"""
class MCD_V7(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MCD_V7, self).__init__(config, dataset)
    ...
    def forward(self,t_feat,v_feat,t_generate_v=None,v_generate_t=None):
        

        


        self.t_embedding = self.w1 * v_generate_t + (1-self.w1)*t_feat
        
        self.v_embedding = self.w2 * t_generate_v + (1-self.w2)*v_feat
    ...
    def calculate_loss(self, interaction):
        ...
        diff_loss1,t_generate_v = self.diffusion_tgi.p_losses(v_feat,t_feat_ori,t) ## 用文本生成的图像表征 在图像空间里
        diff_loss2,v_generate_t = self.diffusion_igt.p_losses(t_feat,v_feat_ori,t) ## 用图像生成的文本表征 在文本空间里 
        diff_loss = self.diff_weight*(diff_loss1+diff_loss2)
        ssl_loss = self.InfoNCE(t_feat,v_feat,self.pos_item)+self.InfoNCE(t_feat,v_feat,self.neg_item)
        ssl_loss = 0.01 * ssl_loss 
        
        self.forward(t_feat,v_feat,t_generate_v,v_generate_t)
    ...
    def full_sort_predict(self, interaction):
        ...
        self.forward(t_feat,v_feat,self.sample_tv,self.sample_vt)
    ...
    def sample(self):
        ...
        t_generate_v = self.diffusion_tgi.sample(v_feat,t_feat_ori) 
        v_generate_t = self.diffusion_igt.sample(t_feat,v_feat_ori)
