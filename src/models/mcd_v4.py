...
class MCD_V4(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MCD_V4, self).__init__(config, dataset)
    ...
    def forward(self,t_feat_ori,v_feat_ori,t_generate_v=None,v_generate_t=None):
        ...
        self.t_embedding = self.w1 * v_generate_t + (1-self.w1)*t_unigraph
        
        self.v_embedding = self.w2 * t_generate_v + (1-self.w2)*v_unigraph
    ...
    def calculate_loss(self, interaction):
        ...
        diff_loss1,t_generate_v = self.diffusion_tgi.p_losses(v_unique,t_feat_ori,t) ## 用文本生成的图像表征 在图像空间里
        diff_loss2,v_generate_t = self.diffusion_igt.p_losses(t_unique,v_feat_ori,t) ## 用图像生成的文本表征 在文本空间里 
        ...
        loss_rec = self.BPR(self.t_rep) + self.BPR(self.v_rep)
