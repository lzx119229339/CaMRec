...
class MCD_V5(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MCD_V5, self).__init__(config, dataset)
        ...
        self.diffusion_tgi = diffusion(config)
        self.diffusion_igt = diffusion(config)
    ...
    def forward(self,t_feat_ori,v_feat_ori,t_generate_v=None,v_generate_t=None):
        
      

        t_unique = self.text_unique(self.t_feat)
        v_unique = self.image_unique(self.v_feat)
        t_unique = torch.sparse.mm(self.text_original_adj,t_unique)
        v_unique = torch.sparse.mm(self.image_original_adj,v_unique)
        
        self.t_unique = self.gcn(self.edge_index,self.t_prefer_unique,t_unique)
        self.v_unique = self.gcn(self.edge_index,self.v_prefer_unique,v_unique)
        
        v_share = torch.sparse.mm(self.image_original_adj,v_feat_ori)
        t_share = torch.sparse.mm(self.text_original_adj,t_feat_ori)

        self.t_embedding = self.w1 * v_generate_t + (1-self.w1)*t_share
        self.v_embedding = self.w2 * t_generate_v + (1-self.w2)*v_share 
    ...
    def calculate_loss(self, interaction):
        ...
        diff_loss1,t_generate_v = self.diffusion_tgi.p_losses(v_share,t_feat_ori,t) ## 用文本生成的图像表征 在图像空间里
        diff_loss2,v_generate_t = self.diffusion_igt.p_losses(t_share,v_feat_ori,t) ## 用图像生成的文本表征 在文本空间里 
        diff_loss = self.diff_weight*(diff_loss1+diff_loss2)
        
        self.forward(t_feat_ori,v_feat_ori,t_generate_v,v_generate_t)
        ...
        loss_rec = self.BPR(self.result_embed) + self.BPR(self.t_unique) + self.BPR(self.v_unique)
        ...
        return loss_rec + reg_loss + diff_loss + ssl_loss

    def full_sort_predict(self, interaction):
        t_feat_ori = self.text_trs(self.t_feat)
        v_feat_ori = self.image_trs(self.v_feat)

        self.forward(t_feat_ori,v_feat_ori,self.sample_tv,self.sample_vt)
    ...
    def sample(self):
        ...
        t_feat_ori = self.text_trs(self.t_feat)
        v_feat_ori = self.image_trs(self.v_feat)
        v_unique = torch.sparse.mm(self.image_original_adj,v_feat_ori)
        t_unique = torch.sparse.mm(self.text_original_adj,t_feat_ori)
        
        t_generate_v = self.diffusion_tgi.sample(v_unique,t_feat_ori) 
        v_generate_t = self.diffusion_igt.sample(t_unique,v_feat_ori)
        


        self.sample_tv = t_generate_v.detach()
        self.sample_vt = v_generate_t.detach()
